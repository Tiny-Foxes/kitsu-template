...
local function ApplySPMods(mod, percent, pn)
    if mod then
		if mod:sub(2):lower() ~= 'mod' then
		...
		if pn then
			...
			for p = 1, #POptions do
				POptions[p][mod](POptions[p], percent, 9e9)
			end
		end
	end
end
        ...
        for i, m in ipairs(b) do
            for j, v in ipairs(m.Modifiers) do
                ...
                if BEAT >= m.Start and BEAT < (m.Start + m.Length) then
					if m.Type == 'Player' or m.Type == 'Special' then
						...
						elseif m.Type == 'Special' then
							ApplySPMods(v[2], mod_percents[pn][v[2]], pn)
						end
...
-- Insert a special test mod.
local function InsertSPMod(self, start, len, ease, modtable, offset, pn)
    --printerr('Mods:InsertMod')
    local t1, t2 = {}, {}
    if not offset or offset == 0 then
		t1 = {
...
local function Invert(t)
	...
	branch:InsertSPMod(t[1], t[2], t[3], {{t[4], 'Invert'}}, nil, t[5])
	branch:AddToModTree()
end
